...
本文档是当前项目的执行入口，用于统一人和 AI 在内容生产流程中的协作方式。当前优先级只覆盖阶段 0 到阶段 3：

- 阶段 1：`case-writer-hybrid` -> `generate-image` -> `wechat-formatter`
- 阶段 2：`wechat-collect` -> 阶段 1 主链路
...
| 公众号文章再创作 | `wechat-collect` -> `case-writer-hybrid` -> `generate-image` -> `wechat-formatter` | 下一阶段 |
